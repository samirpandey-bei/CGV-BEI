...
class Game:
    def __init__(self):
        ...
        self.score_text_rect = self.score_text.get_rect(center=(100, 25))
        self.restart_text = self.font.render("Restart", True, (255, 0, 0))
        self.restart_text_rect = self.restart_text.get_rect(center=(210, 600))
    ...
    def checkScore(self):
        if len(self.pipes) > 0:
            if (
                ...
                and not self.start_monitoring
            ):
                self.start_monitoring = True

            if (
                self.bird.rect.left > self.pipes[0].rect_down.right
                and self.start_monitoring
            ):
                self.start_monitoring = False
                self.score += 1

                if self.score % 10 == 0:
                    pg.mixer.music.load("assets/sfx/abc.mp3")
                    pg.mixer.music.play()
    ...
    def setUpBgAndGround(self):
        ...
        self.ground2_rect.x = self.ground1_rect.right
        self.ground1_rect.y = self.height - 100
        self.ground2_rect.y = self.height - 100
